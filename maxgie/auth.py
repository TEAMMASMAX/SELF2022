...
class Auth(object):
    ...
    def loginWithQrCode(self, certificate=None):
        ...
        self.secondary = Session(self.server.LINE_HOST_DOMAIN, Headers, self.server.LINE_SECONDARY_PATH).QrCode()

        session = self.secondary.createSession(CreateQrSessionRequest()).authSessionId
        e2ee = E2EE()
        secretUrl = e2ee.generateParams()
        qrcode = self.secondary.createQrCode(CreateQrCodeRequest(session))
        self.callback.QrUrl(qrcode.callbackUrl+secretUrl,True)
        Headers = {
            "X-Line-Application": self.appName,
            "User-Agent": self.server.Headers["User-Agent"],
            "X-lal": "en_US",
        ...
        self.permit = Session(self.server.LINE_HOST_DOMAIN, Headers, self.server.LINE_SECONDARY_PERMIT_PATH).QRPermit()

        self.permit.checkQrCodeVerified(CheckQrCodeVerifiedRequest(session))
        try:
            self.secondary.verifyCertificate(VerifyCertificateRequest(session, None if certificate is None else certificate))
        except:
            pincode = self.secondary.createPinCode(CreatePinCodeRequest(session)).pinCode
            print("PIN :", pincode)
            pin = self.permit.checkPinCodeVerified(CheckPinCodeVerifiedRequest(session))
        result = self.secondary.qrCodeLogin(QrCodeLoginRequest(session, self.systemName, True))
