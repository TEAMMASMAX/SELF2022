...
from login import SecondaryQrCodeLoginService, SecondaryQrCodeLoginPermitService
...
class Session:
    ...
    def QrCode(self, isopen=True):
        self.transport = THttpClient(self.host, customThrift=self.customThrift)
        self.transport.setCustomHeaders(self.headers)

        self.protocol = TCompactProtocol.TCompactProtocol(self.transport)
        self._qr  = SecondaryQrCodeLoginService.Client(self.protocol)
    ...
    def QRPermit(self, isopen=True):
        self.transport = THttpClient(self.host, customThrift=self.customThrift)
        self.transport.setCustomHeaders(self.headers)

        self.protocol = TCompactProtocol.TCompactProtocol(self.transport)
        self._qr  = SecondaryQrCodeLoginPermitService.Client(self.protocol)
